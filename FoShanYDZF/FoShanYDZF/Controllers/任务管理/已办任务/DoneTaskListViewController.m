...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *zflx = [NSString stringWithFormat:@"任务类型：%@", [dict objectForKey:@"LCMC"]];
    NSString *rwqk = [NSString stringWithFormat:@"开始时间：%@ 任务发起人：%@", [dict objectForKey:@"LCKSSJ"], [dict objectForKey:@"LCCJR"]];
    return [UITableViewCell makeSubCell:tableView andWithTitle1:dwmc andWithTitle2:zflx andWithTitle3:rwqk andWithTaskType:self.selectedTaskTypeCode];
}
...
- (void)initQueryView
{
    self.taskTypeField.text = [self.taskTypeNameAry objectAtIndex:2];
...
- (void)initTaskTypeData
{
    ...
    self.selectedTaskTypeCode = [self.taskTypeCodeAry objectAtIndex:2];
}

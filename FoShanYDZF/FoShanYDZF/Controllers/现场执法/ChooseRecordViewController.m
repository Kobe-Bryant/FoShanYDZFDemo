...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    /*
     RecordsHelper *dbHelper = [[RecordsHelper alloc] init];
     
     self.chooseAry = [dbHelper queryRecordByWrymc:wrymc andTableName:blName TYPE:type];
...
{
    NSLog(@"%@", self.blName);
    if ([self.blName isEqualToString:@"T_YDZF_XCHJJCZGYJS"])
    {
        //现场监察整改意见书
    ...
    else if ([self.blName isEqualToString:@"T_YDZF_WRYXCJCJLB"])
    {
        //现场监察笔录
        NSMutableDictionary *params = [[NSMutableDictionary alloc] init];
    ...
    else  if ([self.blName isEqualToString:@"T_YDZF_XCJCJL"])
    {
        //动态表单
        NSMutableDictionary *params = [[NSMutableDictionary alloc] init];
    ...
    else
    {
        //动态表单
        NSMutableDictionary *params = [[NSMutableDictionary alloc] init];
        [params setObject:@"QUERY_ZFBD_LSJL" forKey:@"service"];
        [params setObject:self.wrybh forKey:@"WRYBH"];
        [params setObject:@"" forKey:@"templateId"];
        NSString *urlStr = [ServiceUrlString generateUrlByParameters:params];
        self.webHelper = [[NSURLConnHelper alloc] initWithUrl:urlStr andParentView:self.view delegate:self];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.blName isEqualToString:@"T_YDZF_DCXWBL"])
    {
        //询问笔录
        NSString *jcrStr = [dic objectForKey:@"XWR"];
    ...
    else if ( [self.blName isEqualToString:@"T_YDZF_WRYXCJCJLB"])
    {
        ...
        NSString *jcsjStr = [dic objectForKey:@"ZFSJ"];
        cell.textLabel.text = [dic objectForKey:@"WRYMC"];
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@(%@)",jcrStr,jcsjStr];
    }
    else if ([self.blName isEqualToString:@"T_YDZF_XCKCBL"])
    ...
    else if ([self.blName isEqualToString:@"T_YDZF_FJB"])
    {
        ...
        cell.textLabel.text = [dic objectForKey:@"FJMC"];
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@(%@)",jcrStr,jcsjStr];
    }
    else if ( [self.blName isEqualToString:@"T_YDZF_XCHJJCZGYJS"])
    {
        //现场环境监察整改意见书
        NSString *jcrStr = [dic objectForKey:@"JCR"];
        cell.textLabel.text = [dic objectForKey:@"WRYMC"];
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@",jcrStr];
    }
